...
// ==================== 
// BUTTON FIX - Виправлення кнопок форм - ДОДАНО
// ==================== 

// Виправлення конфлікту між onclick і form submit
document.addEventListener('DOMContentLoaded', function() {
    // Виправлення кнопок ban/unban
    const actionForms = document.querySelectorAll('form[action*="ban"], form[action*="unban"]');
    actionForms.forEach(form => {
        form.addEventListener('submit', function(e) {
            const button = this.querySelector('button[type="submit"]');
            if (button && button.classList.contains('btn-action-danger')) {
                if (!confirm('Ви впевнені, що хочете виконати цю дію?')) {
                    e.preventDefault();
                    return false;
                }
...
// Додавання валідації форм
document.querySelectorAll('form').forEach(form => {
    // Пропускаємо форми з action (ban/unban/etc)
    ...
    form.addEventListener('submit', function(e) {
        ...
        requiredInputs.forEach(input => {
            if (!input.value.trim()) {
                ...
                if (!input.nextElementSibling || !input.nextElementSibling.classList.contains('invalid-feedback')) {
                    ...
                    feedback.textContent = 'Це поле обов\'язкове';
        ...
        if (!isValid) {
            e.preventDefault();
            showNotification('Будь ласка, заповніть всі обов\'язкові поля', 'danger');
        }
    });
});
...
console.log('%c✅ All fixes applied successfully!', 'color: #10b981; font-weight: bold; font-size: 14px');
